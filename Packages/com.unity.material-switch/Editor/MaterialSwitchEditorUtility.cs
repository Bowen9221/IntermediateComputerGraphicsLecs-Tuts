...
public static class MaterialSwitchEditorUtility {
    ...
    internal static MaterialProperties CreateMaterialProperties(Material[] materials) {
        ...
        foreach (var (material, mp) in materialProperties) {
            if (mp.propertyFlags.HasFlag(UnityEngine.Rendering.ShaderPropertyFlags.HideInInspector))
                continue;
            if (mp.propertyFlags.HasFlag(UnityEngine.Rendering.ShaderPropertyFlags.PerRendererData))
                continue;
            var displayName = mp.displayName;

            if (mp.propertyType == UnityEngine.Rendering.ShaderPropertyType.Color) {
            ...
            if (mp.propertyType == UnityEngine.Rendering.ShaderPropertyType.Texture) {
            ...
            if (mp.propertyType == UnityEngine.Rendering.ShaderPropertyType.Float) {
            ...
            if (mp.propertyType == UnityEngine.Rendering.ShaderPropertyType.Range) {
                ppm.floatProperties.Add(
                    new RangeProperty() {
                        propertyName = mp.name,
                        propertyId   = Shader.PropertyToID(mp.name),
